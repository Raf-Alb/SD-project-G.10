...
def create_stone(stones, min_distance=200):
    while True:
...
stones = []
for _ in range(3):
    stones.append(create_stone(stones, min_distance=200))  # Spécifier la distance minimale de 200 pixels sur l'axe X
...
while True:
    ...
    for stone in stones:
        ...
        if stone["x"] + stone["width"] < 0:
            new_stone = create_stone(stones, min_distance=200)
            stone["x"] = new_stone["x"]
